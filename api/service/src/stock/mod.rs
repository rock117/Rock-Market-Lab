...
pub async fn get_stock_overviews(page: usize, page_size: usize, sort_by: &str, order: &str, conn: &DatabaseConnection) -> anyhow::Result<StockOverviewResponse> {
    ...
    for stock in &stocks {
        ...
        views.push(create_stock_overview(&stock, &stock_daily_basic, &finance_indicator, &prices, this_year_begin_price, list_date_price, pct_chg, amount));
        curr += 1;
        if curr % 50 == 0 {
