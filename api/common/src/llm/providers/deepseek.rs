...
impl DeepSeekProvider {
    ...
    pub fn new(config: LlmConfig) -> LlmResult<Self> {
        ...
        // 创建 OpenAI 配置，使用 DeepSeek 的 API 端点
        ...
        if let Some(endpoint) = &config.api_endpoint {
            ...
            openai_config = openai_config.with_api_base("https://api.deepseek.com/v1");
        }
    ...
    fn convert_model_name(&self, model: &str) -> String {
        match model {
            "gpt-3.5-turbo" | "gpt-4" | "deepseek-chat" => "deepseek-chat".to_string(),
...
impl LlmProviderTrait for DeepSeekProvider {
    ...
    async fn chat_completion(&self, mut request: ChatCompletionRequest) -> LlmResult<ChatCompletionResponse> {
        ...
        // 转换模型名称
        request.model = self.convert_model_name(&request.model);
        ...
        if self.config.enable_logging.unwrap_or(false) {
            debug!("Sending DeepSeek chat completion request");
        }
        ...
        if self.config.enable_logging.unwrap_or(false) {
            debug!("Received DeepSeek response");
        }
    ...
    async fn chat_completion_stream(
        ...
        // DeepSeek 流式实现
        Err(LlmError::UnknownError("DeepSeek streaming not implemented yet".to_string()))
    }
    
    async fn list_models(&self) -> LlmResult<ModelListResponse> {
        if self.config.enable_logging.unwrap_or(false) {
            debug!("Fetching DeepSeek models");
        }
        ...
        if self.config.enable_logging.unwrap_or(false) {
            info!("Found {} DeepSeek models", models.data.len());
        }
