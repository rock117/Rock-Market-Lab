    ...
    <div class="control-panel">
      ...
      <div class="security-section">
        <el-select
          ...
          filterable
          remote
          :remote-method="(query) => handleSearch(query, '1')"
          :loading="searching"
          placeholder="选择证券1"
          style="width: 250px"
        >
          <el-option
            v-for="item in security1Options"
            :key="item.ts_code"
        ...
        <el-select
          ...
          filterable
          remote
          :remote-method="(query) => handleSearch(query, '2')"
          :loading="searching"
          placeholder="选择证券2"
          style="width: 250px"
        >
          <el-option
            v-for="item in security2Options"
            :key="item.ts_code"
            :label="`${item.ts_code} - ${item.name}`"
            :value="item.ts_code"
          />
...
const updateChart = (data) => {
  ...
  const { dates, prices1, prices2, name1, name2 } = data
  const option = {
    title: {
      text: `${name1} vs ${name2} 价格对比`
    },
    tooltip: {
      trigger: 'axis',
      axisPointer: {
...
const fetchSecurityData = async () => {
  if (!startDate.value || !endDate.value || !security1.value || !security2.value || !selectedPeriod.value) {
    return
  }
  ...
  try {
    ...
    const sec2 = security2Options.value.find(s => s.ts_code === security2.value)

    const [response1, response2] = await Promise.all([
      getSecurityPrice(sec1.type, security1.value, startDate.value, endDate.value),
      getSecurityPrice(sec2.type, security2.value, startDate.value, endDate.value)
    ])
    response1.data.sort((a, b) => a.trade_date.localeCompare(b.trade_date))
    response2.data.sort((a, b) => a.trade_date.localeCompare(b.trade_date))
    const dates = response1.data.map(item => item.trade_date)
    const prices1 = response1.data.map(item => item.close)
    const prices2 = response2.data.map(item => item.close)
...
// 监听输入变化
watch(
  [startDate, endDate, security1, security2, selectedPeriod],
  ([newStartDate, newEndDate, newSecurity1, newSecurity2, newPeriod]) => {
    if (newStartDate && newEndDate && newSecurity1 && newSecurity2 && newPeriod) {
      fetchSecurityData()
    }
  }
)
