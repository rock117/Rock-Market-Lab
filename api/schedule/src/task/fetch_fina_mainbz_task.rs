...
impl Task for FetchFinaMainbzTask {
    ...
    async fn run(&self) -> anyhow::Result<()> {
        ...
        let mut curr = 0;
        for stock in &stocks {
            let incomes = ext_api::tushare::fina_mainbz(&stock.ts_code, &start_date, &end_date).await;
            if let Err(e) = incomes {
                error!("fetch finance_main_business failed, ts_code: {}, error: {:?}", stock.ts_code, e);
                continue;
            }
            let tx = self.0.begin().await?;
            for finance_main_business in incomes? {
                let active_model = entity::finance_main_business::ActiveModel { ..finance_main_business.clone().into() };
