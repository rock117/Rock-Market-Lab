...
impl VolatilityMetrics {
    ...
    pub fn compare(&self, other: &VolatilityMetrics) -> i8 {
        let self_score = self.volatility();
        let other_score = other.volatility();
...
pub fn rank_by_volatility<'a>(
    stocks: &[(&'a str, &[DailyTradeRecord])],
) -> Vec<(&'a str, VolatilityMetrics)> {
    let mut results: Vec<(&'a str, VolatilityMetrics)> = stocks
        .iter()
        .map(|(code, prices)| (*code, calculate_volatility(prices)))
        .collect();
...
mod tests {
    ...
    fn test_basic_volatility_calculation() {
        ...
        assert!(
            (metrics.avg_daily_volatility - expected_avg_daily).abs() < 0.1,
            "日波动率计算错误"
        );
    }
    ...
    fn test_rank_by_volatility() {
        ...
        let stocks = vec![("000001.SZ", &data1[..]), ("000002.SZ", &data2[..])];
