...
pub async fn stock_price(ts_code: &str, start: &str, end: &str, conn: &State<DatabaseConnection>) -> Json<WebResponse<Vec<stock_daily::Model>>> {
    ...
    let start = NaiveDate::parse_from_str(start, common::date::FORMAT).map_err(|e| anyhow!(e)).unwrap();
    let end = NaiveDate::parse_from_str(end, common::date::FORMAT).map_err(|e| anyhow!(e)).unwrap();
